		...
		<tr>
			<td>
				  <?php
					...
					if($dirok) {
					?>
					<a href="<?php echo $_SERVER['PHP_SELF'].'?dir='.urlencode($dotdotdir); ?><?=$linkparm?>"><img src="images/dirup.png" alt="Folder" border="0" /> <strong>..</strong></a><br>
					<?php
						if($class=='b') $class='w';
						else $class = 'b';
					}
					...
					?>
					<a href="<?php echo $_SERVER['PHP_SELF'].'?dir='.urlencode($leadon.$dirs[$i]); ?><?=$linkparm?>"><img src="images/folder.png" alt="<?php echo $dir; ?>" border="0" /> <strong><?php echo $dir; ?></strong></a><br>
					<?php
						if($class=='b') $class='w';
						else $class = 'b';	
					}
					...
					for($i=0;$i<$arsize;$i++) {
						...
						if(in_array($ext, $supportedextentions)) {
							...
							$filedir = str_replace($rootdir,$imagebaseurl, $leadon);
					?>
					<a href="javascript:void(0)" onClick="selectImage('<?php echo $filedir.$filename; ?>');"><img src="images/<?php echo $icon; ?>" alt="<?php echo $files[$i]; ?>" /> <strong><?php echo $filename; ?></strong></a><br>
					<?php
							if($class=='b') $class='w';
							else $class = 'b';	
						}
