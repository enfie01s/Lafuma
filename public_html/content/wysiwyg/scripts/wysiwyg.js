...
var WYSIWYG = {
	 ...
	 */
	Settings: function() {
		this.imagePaths = imageParams;/*set from index.php <script type="text/javascript">var imageParams="<?=$uploaddir?>|<?=$imagebaseurl?>|<?=$rootpath?>"</script>*/
		
		// Images Directory
		this.ImagesDir = "content/wysiwyg/images/";
		
		// Popups Directory
		this.PopupsDir = "content/wysiwyg/popups/";
		
		// CSS Directory File
		this.CSSFile = "content/wysiwyg/styles/wysiwyg.css";		
	...
	_generate: function(n, settings) {
		...
		if(textarea.style.background) {
			this.config[n].Background = textarea.style.backgroundColor
		}
		if(textarea.style.color) {
			this.config[n].Color = textarea.style.color
		}
	    // determine the width + height
		var currentWidth = this.config[n].Width;
		var currentHeight = this.config[n].Height;
	 	var currentBackground = this.config[n].Background;
		var currentColor = this.config[n].Color;
		
		var extraStyle="color:"+currentColor+";background:"+currentBackground;
		// Calculate the width + height of the editor 
		...
		WYSIWYG_Core.setAttribute(doc.body, "style", this.config[n].DefaultStyle+extraStyle);
	...
	execCommand: function(n, cmd, value) {
		...
		switch(cmd) {
			...
			// InsertImage
			case "InsertImage": 
				window.open(imagePopupFile + '?wysiwyg=' + n + '&imagepaths='+this.config[n].imagePaths, 'popup', 'location=0,status=0,scrollbars=0,resizable=0,width=' + imagePopupWidth + ',height=' + imagePopupHeight + ',top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
...
var WYSIWYG_Table = {
	...
	_enableHighlighting: function(node) {
		...
		WYSIWYG_Core.setAttribute(node, "style", "border:1px dashed #AAAAAA;font-size:12px;");
	},
