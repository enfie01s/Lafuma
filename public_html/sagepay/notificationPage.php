...
$strSQL = "SELECT `SecurityKey` FROM orders WHERE `VendorTxCode`='" . mysql_real_escape_string($strVendorTxCode) . "' AND `VPSTxId`='" . mysql_real_escape_string($strVPSTxId) . "'";
	...
	** if you need clarification on this process **/
	...
	if ($strMySignature!==$strVPSSignature)
	{
		...
		** send back a Status of INVALID and failure page RedirectURL **/
		...
		$strSQL = "UPDATE orders SET `Status`='TAMPER WARNING! Signatures do not match for this Order.  The Order was Cancelled. strMySignature=" . $strMySignature . " strVPSSignature=" . $strVPSSignature ."' 
		...
		/** Only use the Internal FQDN value during development.  In LIVE systems, always use the actual FQDN **/
		echo "RedirectURL=" .  $strYourSiteFQDN . $strVirtualDir . "index.php?p=orderFailed&reasonCode=002" . $eoln;
	...
	else
	{
		...
		$strSQL="UPDATE orders SET `pay_method`='".(strtolower($strCardType)=="paypal"?"paypal":"cc")."', `Status`='" . $strDBStatus . "'";
		...
		if ($strStatus=="OK" || $strStatus=="AUTHENTICATED" || $strStatus=="REGISTERED")
			...
			$strRedirectPage="index.php?p=orderSuccessful&VendorTxCode=" . $strVendorTxCode;
		else
			/** The status indicates a failure of one state or another, so send the customer to orderFailed instead **/
			$strRedirectPage="index.php?p=orderFailed&VendorTxCode=" . $strVendorTxCode;
